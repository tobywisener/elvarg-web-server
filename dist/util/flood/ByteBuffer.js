...
var ByteBuffers = exports.ByteBuffers = /** @class */ (function () {
    function ByteBuffers() {
    }
    ByteBuffers.RSA_MODULUS = new big_int_1.BigInt("");
    ByteBuffers.RSA_EXPONENT = new big_int_1.BigInt("65537");
    ByteBuffers.pkt_opcode_slot = 0;
    ByteBuffers.pkt_size_slot = 1;
    ByteBuffers.pkt_content_start = 2;
    ByteBuffers.BIT_CONSTANTS = [0, 1, 3, 7, 15, 31, 63, 127,
...
var ByteBuffer = exports.ByteBuffer = /** @class */ (function () {
    ...
    ByteBuffer.prototype.encryptRSAContent = function () {
         ...
         */
        var decodedBigInteger = new big_int_1.BigInt(decodeBuffer);
        /*
         ...
         * {@link #RSA_MODULES}
         */
        var encodedBigInteger = decodedBigInteger.modPow(ByteBuffers.RSA_EXPONENT, ByteBuffers.RSA_MODULUS);
        /*
         * Returns the value of the {@code #encodedBigInteger} translated to a
         * byte array in big-endian byte-order
         */
        var encodedBuffer = encodedBigInteger.toArray();
    ...
    ByteBuffer.prototype.getShort2 = function () {
        this.position += 2;
        var i = ((this.buffer[this.position - 2] & 0xFF) << 8) + (this.buffer[this.position - 1] & 0xFF);
        if (i > 60000) {
            i = -65535 + i;
        }
        return i;
    ...
    ByteBuffer.prototype.getSignedShort = function () {
        this.position += 2;
        var value = ((this.buffer[this.position - 2] & 0xFF) << 8) + (this.buffer[this.position - 1] & 0xFF);
        if (value > 32767) {
            value -= 0x10000;
        }
        return value;
    ...
    ByteBuffer.prototype.method437 = function () {
        this.position += 2;
        var j = ((this.buffer[this.position - 1] & 0xFF) << 8) + (this.buffer[this.position - 2] & 0xFF);
        if (j > 32767) {
            j -= 0x10000;
        }
        return j;
    ...
    ByteBuffer.prototype.method438 = function () {
        this.position += 2;
        var j = ((this.buffer[this.position - 1] & 0xFF) << 8) + (this.buffer[this.position - 2] - 128 & 0xFF);
        if (j > 32767) {
            j -= 0x10000;
        }
        return j;
    ...
    ByteBuffer.prototype.bufferLength = function () {
        ...
        if (this.reserve_packet_slots) {
            this.buffer[ByteBuffer.pkt_size_slot] = (size + this.cipher.nextInt());
        }
        return size;
    };
