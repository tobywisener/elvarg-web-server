...
var Buffer_1 = require("../collision/Buffer");
var FileUtil_1 = require("../../util/FileUtil");
var ObjectIdentifiers_1 = require("../../util/ObjectIdentifiers");
var fs_extra_1 = require("fs-extra");
var ObjectDefinition = exports.ObjectDefinition = /** @class */ (function (_super) {
    ...
    ObjectDefinition.dumpNames = function () {
        var writer = fs_extra_1.default.writeFile("./Cache/object_names.txt");
        for (var i = 0; i < ObjectDefinition.totalObjects; i++) {
            var def = ObjectDefinition.forId(i);
            var name_1 = def == null ? "null" : def.name;
            writer.write("ID: " + i + ", name: " + name_1 + "");
            writer.newLine();
        }
        writer.close();
    };
    ObjectDefinition.prototype.isClippedDecoration = function () {
        return ObjectDefinition.isInteractive || this.clipType == 1 || ObjectDefinition.obstructsGround;
    };
    ObjectDefinition.forId = function (id) {
        var e_1, _a;
        ...
        var objectDef = ObjectDefinition.cache[ObjectDefinition.cacheIndex];
        ...
        try {
            for (var _b = __values(ObjectDefinition.OBELISK_IDS), _c = _b.next(); !_c.done; _c = _b.next()) {
                var obelisk = _c.value;
                if (id == obelisk) {
    ...
    ObjectDefinition.init = function () {
        try {
            var dat = FileUtil_1.FileUtil.readFile(GameConstants_1.GameConstants.CLIPPING_DIRECTORY + "loc.dat");
            var idx = FileUtil_1.FileUtil.readFile(GameConstants_1.GameConstants.CLIPPING_DIRECTORY + "loc.idx");
            ObjectDefinition.stream = new Buffer_1.Buffer(dat);
            var idxBuffer525 = new Buffer_1.Buffer(idx);
            var totalObjects525 = idxBuffer525.readUnsignedWord();
            ObjectDefinition.streamIndices = new Array(totalObjects525);
            var i = 2;
            for (var j = 0; j < totalObjects525; j++) {
                ObjectDefinition.streamIndices[j] = i;
                i += idxBuffer525.readUnsignedWord();
            }
            ObjectDefinition.cache = new Array(20);
            for (var k = 0; k < 20; k++) {
    ...
    ObjectDefinition.prototype.reset = function () {
    ...
    ObjectDefinition.prototype.readValues = function (buffer) {
        while (true) {
            ...
            if (opcode === 0) {
                ...
                if (len > 0) {
                    if (ObjectDefinition.modelIds === null) {
                        ObjectDefinition.modelTypes = new Array(len);
                        ObjectDefinition.modelIds = new Array(len);
                        for (var i = 0; i < len; i++) {
                ...
                var len = buffer.readUnsignedByte();
                ObjectDefinition.modifiedModelColors = new Array(len);
                ObjectDefinition.originalModelColors = new Array(len);
                for (var i = 0; i < len; i++) {
                ...
                var len = buffer.readUnsignedByte();
                ObjectDefinition.modifiedModelTexture = new Array(len);
                ObjectDefinition.originalModelTexture = new Array(len);
                for (var i = 0; i < len; i++) {
                ...
                var len = buffer.readUnsignedByte();
                ObjectDefinition.childrenIDs = new Array(len + 2);
                for (var i = 0; i <= len; ++i) {
    ...
    ObjectDefinition.prototype.getName = function () {
        return this.name;
    };
    ObjectDefinition.prototype.getSizeX = function () {
        return ObjectDefinition.objectSizeX;
    };
    ObjectDefinition.prototype.getSizeY = function () {
        return ObjectDefinition.objectSizeY;
    };
    ObjectDefinition.prototype.hasActions = function () {
        return ObjectDefinition.isInteractive;
    };
    ObjectDefinition.prototype.getSize = function () {
        switch (this.id) {
    ...
    ObjectDefinition.OBELISK_IDS = [14829, 14830, 14827, 14828, 14826, 14831];
