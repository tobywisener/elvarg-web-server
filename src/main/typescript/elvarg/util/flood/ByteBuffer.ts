...
export class ByteBuffers {
  public static RSA_MODULUS = BigInt(
...
export class ByteBuffer {
  ...
  public getString(): string {
    ...
    return new TextDecoder().decode(
      new Uint8Array(this.buffer.slice(i, this.position - 1))
    );
  }
  ...
  public getTribyte(value: number): number {
    this.position += 3;
    return (
      (0xff & (this.buffer[this.position - 3] << 16)) +
  ...
  public method439(): number {
    this.position += 4;
    return (
      ((this.buffer[this.position - 2] & 0xff) << 24) +
  ...
  public putOpcode(i: number): void {
    this.buffer[
      this.reserve_packet_slots ? ByteBuffer.pkt_opcode_slot : this.position++
    ] = (i + this.cipher.nextInt()) as number & 0xff;
  }
  ...
  public bufferLength(): number {
    ...
    if (this.reserve_packet_slots) {
      this.buffer[ByteBuffer.pkt_size_slot] = (size +
        this.cipher.nextInt()) as number & 0xff;
    }
    return size;
  }
