...
export class LoginDecoder {
  ...
  private decodeRequest(buffer: Buffer) {
    ...
    if (buffer.length == 8) {
      ...
      this.hostAddressOverride = `${ip & 0xff}.${(ip >> 8) & 0xff}.${
        (ip >> 16) & 0xff
      }.${(ip >> 24) & 0xff}`;
    }
  ...
  private decodeType(buffer: Buffer) {
    ...
    if (
      ...
      Server.getLogger().info(
        "Session rejected for bad connection type id: " + connectionType
      );
      LoginDecoder.sendLoginResponse(LoginResponses.LOGIN_BAD_SESSION_ID);
      return;
    }
  ...
  private decodeLogin(buffer: Buffer, out: Array<any>) {
    ...
    if (encryptedLoginBlockSize != buffer.length) {
      Server.getLogger().info(
        `[host= ${ipAddress}] encryptedLoginBlockSize != readable bytes`
      );
    ...
    if (buffer.length > 0) {
      ...
      if (magicId != 0xff) {
        Server.getLogger().info(
          `[host= ${ipAddress}] [magic= ${magicId}] was rejected for the wrong magic value.`
        );
        LoginDecoder.sendLoginResponse(LoginResponses.LOGIN_REJECT_SESSION);
        return;
      }
      ...
      if (memory != 0 && memory != 1) {
        Server.getLogger().info(
          `[host= ${ipAddress}] was rejected for having the memory setting.`
        );
        LoginDecoder.sendLoginResponse(LoginResponses.LOGIN_REJECT_SESSION);
        return;
      }
      ...
      if (securityId != 10 && securityId != 11) {
        Server.getLogger().info(
          `[host= ${ipAddress}] was rejected for having the wrong securityId.`
        );
        LoginDecoder.sendLoginResponse(LoginResponses.LOGIN_REJECT_SESSION);
        return;
      }
      ...
      if (securityId == 10) {
        ...
        if (
          ...
          LoginDecoder.sendLoginResponse(
            LoginResponses.INVALID_CREDENTIALS_COMBINATION
          );
          return;
        }

        out.push(
          new LoginDetailsMessage(
            username,
            password,
            host,
        ...
        if (
          rawUsername == DiscordUtil.DiscordConstants.USERNAME_CACHED_TOKEN ||
          rawUsername == DiscordUtil.DiscordConstants.USERNAME_AUTHZ_CODE
        ) {
          let msg = new LoginDetailsMessage(
            rawUsername,
            password,
            host,
            new IsaacRandom(seed),
