...
export class LoginResponses {
  ...
  public static async evaluate(player: any, msg: LoginDetailsMessage) {
    ...
    let playerLoadingResponse = await LoginResponses.getPlayerResult(
      player,
      msg
    );
  ...
  private static async getDiscordResult(
    player: any,
    msg: LoginDetailsMessage
  ): Promise<number> {
    try {
      let discordInfo: DiscordInfo;
      if (
        msg.getUsername() === DiscordUtil.DiscordConstants.USERNAME_AUTHZ_CODE
      ) {
        discordInfo = await DiscordUtil.getDiscordInfoWithCode(
          msg.getPassword()
        );
      } else if (
        msg.getUsername() === DiscordUtil.DiscordConstants.USERNAME_CACHED_TOKEN
      ) {
        if (!DiscordUtil.isTokenValid(msg.getPassword()))
          return LoginResponses.LOGIN_INVALID_CREDENTIALS;
        discordInfo = await DiscordUtil.getDiscordInfoWithToken(
          msg.getPassword()
        );
      } else {
        return LoginResponses.LOGIN_INVALID_CREDENTIALS;
  ...
  private static async getPlayerResult(player: any, msg: LoginDetailsMessage) {
